...
describe('Search functionality', () => {
    ...
    test('should search in title field', () => {
        ...
        expect(result.some(each => each.title === 'Holy, Holy, Holy')).toBe(true);
        expect(result.some(each => each.title === 'Be With Us, Mary')).toBe(false);
    })
    ...
    test.skip('should search multiple words seperately', () => {
        ...
        expect(result.some(each => each.title === 'Let It Be Done Unto Me spanish')).toBe(true);
        expect(result.some(each => each.title === 'Holiness is Faithfulness')).toBe(true);
    })
...
describe('HTML tags sanitisation', () => {
    ...
    test('should ignore html tag - span', () => {
        ...
        expect(result.some(({title}) => title === 'Let It Be Done Unto Me spanish')).toBe(true);
    })
})
